...
class SupplierController extends Controller
{
    ...
    public function save(Request $request, $id = null)
    {
		...
		return view('krishna.supplier', $viewData)->with($getFormAutoFillup);
    }
     ...
     public function view(Request $request)
    {
    	if($request->isMethod('post'))
    	{
			...
			if($request->has('created_at_from') && $request->created_at_from !=''){
                ...
                $Supplier->whereDate('created_at', '<=', $created_at_from);
            }
            if($request->has('created_at_to') && $request->created_at_to !=''){
                $created_at_to = date("Y-m-d", strtotime($request->created_at_to) );
                $Supplier->whereDate('created_at', '>=', $created_at_to);
            }
			if($request->has('contact_no') && $request->contact_no !=''){
				$Supplier->where('contact_no', '=', $request->contact_no);
			}
            if($request->has('address') && $request->address !=''){
                $Supplier->where('address', '=', $request->address);
            }
		    ...
		    return view('krishna.supplier', $viewData);
    	}else
    	{
    		$viewData['pageTitle'] = 'Supplier';       	
			$viewData['supplier'] = Supplier::orderBy('id','desc')->get();
    ...
    public function trashedList()
    {
         ...
         return view('krishna.supplier.delete', compact('TrashedParty', 'TrashedParty')); 
    }
    public function permanemetDelete(Request $request,$id)
    {
    	if(($id!=null) && (Supplier::where('id',$id)->forceDelete())){
    	 ...
    	 $TrashedParty = Supplier::orderBy('deleted_at', 'desc')->onlyTrashed()->simplePaginate(10);
         return view('krishna.supplier', compact('TrashedParty', 'TrashedParty'));
    }
}
